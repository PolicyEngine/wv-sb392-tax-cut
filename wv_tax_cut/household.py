"""Household situation builder for the WV SB 392 calculator.

This module builds PolicyEngine household situations used by the
household calculator view. The calculator contrasts current-law 2026 with
the pre-cut rates described in ``reform_revert.json``; ``impact =
current_law - pre_cut``.
"""
...
def build_household_situation(
    ...
    Args:
        ...
        state_code: Two-letter state code (default: "WV")
...
def calculate_household_impact(
    ...
    """
    Calculate household impact of WV SB 392 using PolicyEngine.

    This is the Python equivalent of the calculator view in the frontend:
    pre-cut rates are the baseline, current law is the reform, and impact
    is current law minus pre-cut law.
    ...
    Returns:
        ...
        wv_income_tax_change, and income_tax_change (federal).
    """
    ...
    baseline_wv_income_tax = baseline_sim.calculate("wv_income_tax", year)
    ...
    reform_wv_income_tax = reform_sim.calculate("wv_income_tax", year)
    reform_income_tax = reform_sim.calculate("income_tax", year)

    net_income_change = reform_net_income - baseline_net_income
    wv_income_tax_change = reform_wv_income_tax - baseline_wv_income_tax
    ...
    return {
        ...
        "wv_income_tax_change": wv_income_tax_change.tolist(),
